...
AudioStreamer::AudioStreamer() 
    : m_lastRateUpdate(std::chrono::steady_clock::now()) {
}
...
void AudioStreamer::SetAudioSource(const std::string& sourceName) {
    ...
    if (m_audioSourceName == sourceName) return;
...
void AudioStreamer::AttachAudioSource() {
    ...
    if (m_audioSourceName.empty()) return;
    
    DetachAudioSource();
    
    m_audioSource = obs_get_source_by_name(m_audioSourceName.c_str());
    if (!m_audioSource) {
        emit errorOccurred(QString("Audio source '%1' not found").arg(QString::fromStdString(m_audioSourceName)));
        return;
    }
...
void AudioStreamer::AudioCaptureCallback(void* param, obs_source_t* source, 
